const STORAGE_KEY = 'nanogpt_sidepanel_conversation';
...
async function buildReportDiagnostics() {
  ...
  return formatDiagnosticsForReport({
    manifest: chrome.runtime.getManifest ? chrome.runtime.getManifest() : {},
...
async function signInWithNanoGPT() {
  ...
  try {
    ...
    chrome.tabs.create({ url: startData.verification_uri_complete });
...
function isReceivingEndMissing(errorMessage) {
  return typeof errorMessage === 'string' && errorMessage.includes('Receiving end does not exist');
}
...
async function sendActiveTabMessage(message) {
  ...
  return new Promise((resolve) => {
    ...
    sendTabMessage(tab.id, message).then(async (response) => {
      if (response.ok || !isReceivingEndMissing(response.error)) {
        resolve(response);
        return;
      }
      ...
      resolve(await sendTabMessage(tab.id, message));
    });
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (!message || message.action !== 'nanogpt_page_action_state') return;
